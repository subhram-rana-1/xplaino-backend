...
- Always prioritize accuracy and relevance over attempting to answer every question"""
    ...
    async def generate_contextual_answer_stream(self, question: str, chat_history: List, initial_context: Optional[Union[str, Dict[str, str]]] = None, language_code: Optional[str] = None, context_type: Optional[str] = "TEXT"):
        ...
        Args:
            ...
            initial_context: Optional initial context: plain text string or dict mapping IDs to text (e.g. {"1": "..."}).
            language_code: Optional target language code. If provided, response will be in this language.
        ...
        """
        try:
            # Normalize initial_context to a string for language detection
            context_for_language = None
            if initial_context is not None:
                context_for_language = initial_context if isinstance(initial_context, str) else "\n\n".join(initial_context.values())
...
- Do NOT use any other language"""
                ...
                if context_for_language:
                    text_to_detect += " " + context_for_language
...
- Format complex information in a way that makes it easy to scan and digest"""
            ...
            # Add source reference instructions and initial context (branch on str vs dict)
            source_reference_instructions = ""
            if initial_context is not None:
                if isinstance(initial_context, str):
                    # Plain text: substring-based references when PAGE
...
REMEMBER: When context_type = PAGE, you MUST include source references in the format [[[(N)substring]]] for important points. This is MANDATORY."""
                    system_content += f"\n\nInitial Context (SOURCE FOR REFERENCES): {initial_context}\n\nPlease use this context to provide more informed and relevant answers. When you mention important points that users should verify, include source references in the format [[[(N)substring from initial_context]]]." if context_type == "PAGE" else f"\n\nInitial Context: {initial_context}\n\nPlease use this context to provide more informed and relevant answers to questions about this topic."
                else:
                    # Dict (JSON): content as-is, ID-based references when PAGE
                    initial_context_json = json.dumps(initial_context, indent=2)
                    system_content += f"\n\nInitial Context (each key is an ID for references):\n{initial_context_json}\n\nPlease use this context to provide more informed and relevant answers to questions about this topic."
                    if context_type == "PAGE":
                        content_ids_example = ", ".join(f'"{k}"' for k in list(initial_context.keys())[:5])
                        if len(initial_context) > 5:
                            content_ids_example += ", ..."
                        source_reference_instructions = f"""

⚠️⚠️⚠️ CRITICAL: SOURCE REFERENCE REQUIREMENTS (context_type = PAGE) - THIS IS MANDATORY ⚠️⚠️⚠️

When you mention important points, facts, claims, or specific information in your answer that the user should verify, you MUST include source references using the IDs (keys) from the initial context object above.

SOURCE REFERENCE FORMAT (USE EXACTLY 3 BRACKETS [[[):
- After mentioning an important point that needs verification, immediately include a source reference
- Format: [[[ref:("id1","id2")]]] or for a single ID: [[[ref:("id1")]]]
- Use EXACTLY THREE opening brackets [[[ and THREE closing brackets ]]]
- The IDs inside ref:(...) MUST be keys from the initial context object above (e.g. {content_ids_example})
- You MAY include multiple IDs in one ref when several context blocks support the same point (e.g. [[[ref:("1","2")]]])
- Stream the reference as a SINGLE complete event - do not break it up

EXAMPLES (CORRECT FORMAT):
- Single ID: "The discovery was made in 1923. [[[ref:("1")]]]"
- Multiple IDs for one point: "The report states both findings. [[[ref:("1","3")]]]"

IMPORTANT RULES:
- You MUST include source references for important, verifiable points - this is NOT optional when context_type = PAGE
- Use ONLY IDs that exist as keys in the initial context object above
- ALWAYS use THREE brackets: [[[ and ]]] - never use two brackets [[

REMEMBER: When context_type = PAGE, you MUST include source references in the format [[[ref:("id1","id2")]]] for important points. This is MANDATORY."""
...
Summary:"""
    ...
    async def summarise_text_stream(self, content: Dict[str, str], language_code: Optional[str] = None, context_type: Optional[str] = "TEXT"):
        """Generate a short, insightful summary of the given content with streaming.

        Args:
            content: Object mapping IDs to text (e.g. {"1": "text...", "2": "text..."}). Sent as-is to the model; references in the summary use these IDs.
            language_code: Optional target language code. If provided, response will be in this language.
                          If None, language will be detected from the concatenated content.
            context_type: Type of context - "PAGE" (for page/document context with source references) or "TEXT" (standard text context). Default is "TEXT".

        Yields:
            Chunks of the summary text as they are generated by OpenAI. References use format [[[ref:("id1","id2")]]].
        """
        try:
            combined_text = "\n\n".join(content.values())
...
"""
                ...
                # Case 1: languageCode is None - detect language from concatenated content
                detected_language_code = await self.detect_text_language_code(combined_text)
                detected_language_name = get_language_name(detected_language_code)
...
"""
            ...
            # Build the main instruction: content is sent as-is (JSON). References use IDs from the keys.
            if context_type == "PAGE":
                main_instruction = f"""{language_requirement}⚠️ CRITICAL: You are summarizing with context_type = PAGE. You MUST include source references for important points using the IDs from the content below. ⚠️

Analyze the following content and generate a short, insightful summary that captures the main ideas and key points. When you mention important facts, claims, or specific information, you MUST include source references in the format [[[ref:("id1","id2")]]] using the keys (IDs) from the content object below. Multiple IDs may be used for one point.

Content to summarize (each key is an ID for references):
{content_json}"""
            else:
                main_instruction = f"""{language_requirement}⚠️ CRITICAL: You MUST include source references for important points in your summary using the IDs from the content below. ⚠️
...
CRITICAL REQUIREMENTS:
- Generate a concise summary that captures the essence and main ideas of the content
- Keep it brief but insightful - focus on the most important information
- Preserve the core meaning and key concepts
- Make it clear and easy to understand
- If the content has multiple sections (multiple keys), synthesize them into a coherent summary
- The summary should be significantly shorter than the original content while retaining key information
- ⚠️ MANDATORY: You MUST include source references in the format [[[ref:("id1","id2")]]] for important, verifiable points - use the IDs (keys) from the content object above. This is REQUIRED for ALL summaries.
- You MAY include multiple IDs in one reference when several parts support a single point (e.g. [[[ref:("1","3")]]] after one sentence).
...
- Format the response using Markdown syntax (**, *, bullet points, etc.) combined with emojis for the best reading experience"""
            ...
            # Add source reference instructions - ALWAYS included for all summaries (ID-based refs from content keys)
            content_ids_example = ", ".join(f'"{k}"' for k in list(content.keys())[:5])
            if len(content) > 5:
                content_ids_example += ", ..."
...
You MUST include source references when mentioning important points, facts, claims, or specific information in your summary. Use the IDs (keys) from the content object above. This is REQUIRED for ALL summaries.

SOURCE REFERENCE FORMAT (USE EXACTLY 3 BRACKETS [[[):
- After mentioning an important point that needs verification, immediately include a source reference
- Format: [[[ref:("id1","id2")]]] or for a single ID: [[[ref:("id1")]]]
- Use EXACTLY THREE opening brackets [[[ and THREE closing brackets ]]]
- The IDs inside ref:(...) MUST be the keys from the content object above (e.g. {content_ids_example})
- You MAY include multiple IDs in one ref when several content blocks support the same point (e.g. [[[ref:("1","2")]]])
- Stream the reference as a SINGLE complete event - do not break it up
- CRITICAL: The reference MUST be streamed in the format [[[ref:("id1",...)]]] - this exact format is MANDATORY

EXAMPLES (CORRECT FORMAT):
- Single ID: "The discovery was made in 1923. [[[ref:("1")]]]"
- Multiple IDs for one point: "The report states both findings. [[[ref:("1","3")]]]"
- Another: "The population increased by 25%. [[[ref:("2")]]]"

IMPORTANT RULES:
- You MUST include source references for important, verifiable points - this is NOT optional
- Include references for key facts, statistics, claims, important dates, names, or specific data
- Do NOT include references for every sentence - focus on important, verifiable information
- A single point or sentence MAY have multiple IDs in one ref when that strengthens verification
- Use ONLY IDs that exist as keys in the content object above
- Stream each reference as a single complete event immediately after the relevant sentence/point
- ALWAYS use THREE brackets: [[[ and ]]] - never use two brackets [[
- The format [[[ref:("id1",...)]]] MUST appear in your streamed response for important points

REMEMBER: You MUST include source references in the format [[[ref:("id1","id2")]]] for important points. This is MANDATORY for ALL summaries. The format must be present in your streamed output."""
...
Questions (JSON array only):"""
    ...
    async def generate_recommended_questions(self, current_question: str, chat_history: List, initial_context: Optional[Union[str, Dict[str, str]]] = None, language_code: Optional[str] = None) -> List[str]:
        ...
        Args:
            ...
            chat_history: Previous chat history for context
            initial_context: Optional initial context: plain text string or dict mapping IDs to text.
            language_code: Optional language code. If provided, questions will be strictly in this language.
...
"""
            ...
            # Build initial context section (string or concatenated dict values)
