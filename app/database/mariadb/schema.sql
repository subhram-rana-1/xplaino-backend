...
-- PDF text chat table (conversation record anchored to a text selection on a PDF)
...
-- PDF text chat history table (individual USER/SYSTEM messages within a conversation)
...
-- PDF content preprocessing table (tracks async embedding pipeline status)
...
-- PDF chat session table (groups conversation messages for a preprocessed PDF)
...
-- PDF chat table (individual messages within a chat session)
